...
NEI<-subset(NEI,year >= "1999" & year<= "2008" & fips == "24510", select=c("year","Emissions","fips"))
...
rm(NEI)
library(ggplot2)
qplot(Group.1, x, data=tbl, color = Group.2, geom=c("point","line"), facets = .~Group.1, group=Group.1)
